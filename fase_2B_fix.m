% INPUT:
%--------
% Data "PC2_15_Split_1_2B_Split" -> Data split 2B
% Data "PC2_17_DataFitur_2B"  -> Data training untuk 2B

% ----------------------------------------------------------------------------------------------------------
% Cari jumlah TRUE dan FALSE serta nilai ENTROPY children di Mtraining berdasarkan "PC2_15_Split_1_2B_Split"
% ----------------------------------------------------------------------------------------------------------                        
   
for iKolomCellB = 1 : 36 % Iterasi fitur PC2 ada 36 (exclude kelas)    
%--    
    panjangSplit2B = length(PC2_15_Split_1_2B_Split{1,iKolomCellB}); % Setiap DATA SPLIT diulang sebanyak jumlah DATA TRAINING
    panjangTraining2B = size(PC2_17_DataFitur_2B{1,iKolomCellB},1); % Iterasi data training AGAR MATCH dengan SATU DATA split
    for iBarisSplitB = 1 : panjangSplit2B % data SPLIT 2B          
        ...
        % -----------------------------                
        dataSplitB = PC2_15_Split_1_2B_Split{1, iKolomCellB}(iBarisSplitB,1); % Data SPLIT 2B
        ...
        %--    
            for iBarisTrainingB = 1 : panjangTraining2B % data TRAINING 2B
                ...
                % -----------------------------------------------------------
                dataTrainingB = PC2_17_DataFitur_2B{1, iKolomCellB}(iBarisTrainingB,1); % Data training                    
                dataKelasB = PC2_17_DataFitur_2B{1, iKolomCellB}(iBarisTrainingB,2); % Data kelas                                 
            ...
            PC2_15_Split_1_2B_Split{1,iKolomCellB}(iBarisSplitB,2) = jmlTrueKurangB; % Jumlah TRUE dengan parameter ( <= ) disimpan di kolom 2
            PC2_15_Split_1_2B_Split{1,iKolomCellB}(iBarisSplitB,3) = jmlFalseKurangB; % Jumlah FALSE dengan parameter ( <= ) disimpan di kolom 3
            PC2_15_Split_1_2B_Split{1,iKolomCellB}(iBarisSplitB,5) = jmlTrueLebihB; % Jumlah TRUE dengan parameter ( > ) disimpan di kolom 5
            PC2_15_Split_1_2B_Split{1,iKolomCellB}(iBarisSplitB,6) = jmlFalseLebihB; % Jumlah FALSE dengan parameter ( > ) disimpan di kolom 6                                
            ...
            PC2_15_Split_1_2B_Split{1,iKolomCellB}(iBarisSplitB,4) = entropyChildKurangB(iBarisSplitB,1); % Nilai entropy child dari parameter ( <= ) disimpan di kolom 4                          
            ...
            PC2_15_Split_1_2B_Split{1,iKolomCellB}(iBarisSplitB,7) = entropyChildLebihB(iBarisSplitB,1); % Nilai entropy child dari parameter ( > ) disimpan di kolom 7                                                    

            % -----------------------------------------
            % Mencari nilai INFO dari setiap data split
            % -----------------------------------------
            dataChildKurangB = (totalKurangB/jmlData) * PC2_15_Split_1_2B_Split{1, iKolomCellB}(iBarisSplitB,4);
            dataChildLebihB = (totalLebihB/jmlData) * PC2_15_Split_1_2B_Split{1, iKolomCellB}(iBarisSplitB,7);
            INFOsplitB(iBarisSplitB,1) = (dataChildKurangB + dataChildLebihB);
            PC2_15_Split_1_2B_Split{1,iKolomCellB}(iBarisSplitB,8) = INFOsplitB(iBarisSplitB,1); % nilai INFO dari data SPLIT. disimpan di kolom 8

            % ------------------------------------
            % Mencari nilai GAIN dari setiap INFO
            % ------------------------------------
            GAINinfoB(iBarisSplitB,1) = PC2_03_Keterangan(1,4) - INFOsplitB(iBarisSplitB,1);
            PC2_15_Split_1_2B_Split{1,iKolomCellB}(iBarisSplitB,9) = GAINinfoB(iBarisSplitB,1); % nilai INFO dari data SPLIT. disimpan di kolom 9                        

            % ----------------------------------------------------------------------------------------------------------------------------
            % Penyederhanaan variable "PC2_15_Split_1_2B_Split" 
    ...
    % -------------------------------------------
    if length(PC2_15_Split_1_2B_Split{1,iKolomCellB}) ~= 0                                        
        % ---------------------------------------------------------------
        % Mencari nilai best split berdasarkan nilai GAIN tertinggi (max)
        % ---------------------------------------------------------------
        [NilaiB,BarisKeB] = max(PC2_15_Split_1_2B_Split{1,iKolomCellB}(:,9)); % Ambil urutan ke berapa si split terbaik itu dan ambil nilai max gain-nya
        angkaSplitB = PC2_15_Split_1_2B_Split{1, iKolomCellB}(BarisKeB,1); % Angka split terbaik dari daftar urut split
        PC2_19_BEST_Split_2B{1,iKolomCellB} = [BarisKeB angkaSplitB NilaiB]; % nilai max Gain dari data split ke berapa                                                                                                                   
    end            
%--
end              
...
% "PC2_15_Split_1_2B_Split" dengan total 9 kolom
% Nilai Gain (MAX) sebagai split terbaik "PC2_19_BEST_Split_2B"        
